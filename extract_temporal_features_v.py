...
OUTPUT_FILE = "temporal_features_v2.csv"
...
def calculate_frame_features(
    ...
    for vehicle_id, vehicle in current.items():
        ...
        if previous is not None and vehicle_id in previous:
            ...
            if (
                ...
                # ------------------------------------------------
                # SUDDEN DECELERATION / SPEED DROP
                # ------------------------------------------------
    ...
    for i in range(len(ids)):
        ...
        for j in range(i + 1, len(ids)):
            ...
            if (
                ...
                if (
                    previous_previous is not None
                    and id1 in previous_previous
                    and id2 in previous_previous
                ):
...
# ============================================================
def aggregate_window(frame_features):
    ...
    result["max_speed_drop"] = df[
        "max_speed_drop"
    ].max()

    result["mean_speed_drop"] = df[
        "mean_speed_drop"
    ].mean()
    ...
    result["max_approach_acceleration"] = df[
        "max_approach_acceleration"
    ].max()

    result["mean_approach_acceleration"] = df[
        "mean_approach_acceleration"
    ].mean()
    ...
    result["max_direction_change"] = df[
        "max_direction_change"
    ].max()

    result["mean_direction_change"] = df[
        "mean_direction_change"
    ].mean()
